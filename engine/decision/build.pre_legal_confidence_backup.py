    ...
    for feat in legal_features:
        ...
        for idx, (fx, fy, weight) in enumerate(_candidate_offsets(), start=1):
            ...
            confidence = max(
                ...
                min(
                    92.0,
                    56.0 + terrain_score * 0.45 + shape_bonus * 1.3 + terrain_read["confidence_bonus"] + legal_confidence_delta + (5.0 if legal_class == "legal" else -10.0) - validity_read["confidence_penalty"] - practical_read["confidence_penalty"],
                ),
            )
