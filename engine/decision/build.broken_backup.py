    ...
    for feat in features:
        ...
        for idx, (fx, fy, weight) in enumerate(_candidate_offsets(), start=1):
            ...
            confidence = max(
                ...
                min(
                    92.0,
                    56.0 + terrain_score * 0.45 + shape_bonus * 1.3 + terrain_read["confidence_bonus"] + (5.0 if legal_class == "legal" else -10.0) - validity_read["confidence_penalty"] - practical_read["confidence_penalty"],
                ),
            )
            ...
            if legal_class == "legal":
                ...
                record["score"] = max(28, record["score"] - (22 if legal_class == "restricted" else 14))
                record["reasoning"] = validity_read.get("summary_reason") or (
                    f"Suppressed because legality is {legal_class} at {name}; shown for operator awareness. "
                    f"{validity_read['reason_bits'][0]}." if validity_read["reason_bits"] else
                    f"Suppressed because legality is {legal_class} at {name}; shown for operator awareness."
                )
                record["primary_tag"] = "Exploration Only"
                record["support_tags"] = [tag for tag in validity_read["display_tags"] if tag not in {"Exploration Only"}][:2]
                record["all_tags"] = ["Exploration Only", *record["support_tags"]][:3]
