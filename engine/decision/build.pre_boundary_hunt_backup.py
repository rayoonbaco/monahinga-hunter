...
def _boundary_hunt_weight(edge_risk: bool, terrain_read: dict) -> tuple[float, float, str | None]:
    ...
    if movement_edge:
        return 2.5, 2.0, "Boundary Funnel"
    if bedding_edge:
        return 1.5, 1.0, "Boundary Bedding Edge"
    return -2.5, -3.0, "Boundary Exposure"
...
def build_decision_artifact(
    ...
    for feat in legal_features:
        ...
        for idx, (fx, fy, weight) in enumerate(_candidate_offsets(), start=1):
            ...
            score += legal_score_delta + boundary_score_delta
            ...
            confidence = max(
                ...
                min(
                    92.0,
                    56.0 + terrain_score * 0.45 + shape_bonus * 1.3 + terrain_read["confidence_bonus"] + legal_confidence_delta + boundary_confidence_delta + (5.0 if legal_class == "legal" else -10.0) - validity_read["confidence_penalty"] - practical_read["confidence_penalty"],
                ),
            )
