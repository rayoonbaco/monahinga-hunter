...
def _boundary_hunt_weight(edge_risk: bool, terrain_read: dict) -> tuple[float, float, str | None]:
    ...
    # softer penalty — don’t kill edges completely
...
def _wildlife_micro_signal(
    ...
    if wind_speed >= 10.0 and movement_lane:
        return 2.0, 1.0, "Wildlife Movement Push"

    if wind_speed <= 5.0 and bedding_hold:
        return 1.0, 0.5, "Wildlife Bedding Hold"
...
def _practical_access_read(
    ...
    gy, gx = np.gradient(broad.astype(np.float32)) if broad.size else (np.zeros((1, 1), dtype=np.float32), np.zeros((1, 1), dtype=np.float32))
    ...
    reason_bits: list[str] = []
    if isolated_landform:
        reason_bits.append("the sit rides on a top that stays elevated above most surrounding directions, which makes the whole landform feel isolated rather than normally huntable")
    if cliff_isolation_risk:
        reason_bits.append("the surrounding drop is severe enough in multiple directions that this behaves more like a cliff-isolated cap than a normal bench")
    if summit_cap_risk and not cliff_isolation_risk:
        reason_bits.append("the point sits high on the landform with enough fall-away around it that a locally calm patch should not be trusted like a normal travel system")
    if peak_position_risk:
    ...
    summary_reason = ""
    if cliff_isolation_risk:
        summary_reason = "This point sits on an isolated cap with severe drop around it, so even if the top reads calm the setup should be treated as a poor practical hunt option rather than a trustworthy default sit."
    elif hard_override:
        summary_reason = "This point sits too high and too isolated on the landform to trust as a default sit, so it should rank behind more connected terrain if any cleaner option exists inside the box."
    elif limited_practical_access:
        summary_reason = "This terrain may read well on the map, but practical access looks limited enough that it should be treated as a higher-effort hunt rather than an easy default sit."
    elif high_effort_access:
        summary_reason = "This sit has usable terrain, but the elevation isolation and surrounding ruggedness make it a higher-effort hunt than the local shelf read alone suggests."
...
def _select_trust_terrain_tag(primary_record: dict) -> str:
    ...
    if primary and primary not in {"Exploration Only", "Terrain Read", "Interior Security", "Legal Interior Access", "Verified Legal Access"}:
...
def _select_trust_modifier_tag(primary_record: dict, alternatives: list[dict], requested_wind: str) -> str | None:
    ...
    alt_access_risk = bool(alt_best.get("limited_practical_access") or alt_best.get("high_effort_access") or alt_best.get("hard_override"))
    primary_access_clean = not bool(primary_record.get("limited_practical_access") or primary_record.get("high_effort_access") or primary_record.get("hard_override"))

    # STRICT MODIFIER RULES (only strong, meaningful advantages)
...
def _select_trust_caution_tag(primary_record: dict, alternatives: list[dict], requested_wind: str) -> str | None:
    # STRICT CAUTION RULES (only real risks)
...
def build_decision_artifact(
    ...
    for feat in legal_features:
        ...
        for idx, (fx, fy, weight) in enumerate(_candidate_offsets(), start=1):
            ...
            if requested_wind:
                ...
                score += 5.0 if wind_fit else -8.0
            ...
            wildlife_score_delta, wildlife_confidence_delta, wildlife_tag = _wildlife_micro_signal(
                terrain_truth_root,
                terrain_read,
            )
            score += legal_score_delta + boundary_score_delta + wildlife_score_delta
            ...
            confidence = max(
                ...
                min(
                    92.0,
                    56.0 + terrain_score * 0.45 + shape_bonus * 1.3 + terrain_read["confidence_bonus"] + legal_confidence_delta + boundary_confidence_delta + wildlife_confidence_delta + (5.0 if legal_class == "legal" else -10.0) - validity_read["confidence_penalty"] - practical_read["confidence_penalty"],
                ),
            )
            ...
            if validity_read["suppress_positive"]:
                ...
                if len(terrain_read["support_tags"]) < 2:
                    for fallback_tag in ["Cliff Isolation Risk", "Limited Practical Access", "Peak Position Risk", "High Effort Access"]:
                ...
                terrain_read["all_tags"] = _merge_display_tags([terrain_read["primary_tag"]], practical_read["display_tags"] + terrain_read["support_tags"])
                terrain_read["support_tags"] = terrain_read["all_tags"][1:3]

            if boundary_tag:
                terrain_read["all_tags"] = _merge_display_tags([terrain_read["primary_tag"]], [boundary_tag] + terrain_read["support_tags"])
                terrain_read["support_tags"] = terrain_read["all_tags"][1:3]

            if wildlife_tag:
                terrain_read["all_tags"] = _merge_display_tags([terrain_read["primary_tag"]], [wildlife_tag] + terrain_read["support_tags"])
                terrain_read["support_tags"] = terrain_read["all_tags"][1:3]
            ...
            if validity_read["suppress_positive"] and validity_read.get("summary_reason"):
                ...
                if validity_read["reason_bits"]:
                    reasoning += " " + " ".join(bit + "." for bit in validity_read["reason_bits"])
    ...
    if not legal_candidates:
        ...
        for idx, (fx, fy, name) in enumerate(sample_points, start=1):
            ...
            score = 54.0 + terrain_score - validity_read["score_penalty"] - idx
            fallback.append({
                "lon": float(lon),
                "lat": float(lat),
                "elevation_m": float(sample["elevation_m"]),
                ...
                "reasoning": validity_read.get("summary_reason") or (terrain_read["reasoning"] + " Legal hunting access is not verified here, so use this as exploration guidance only." + (" " + " ".join(bit + "." for bit in validity_read["reason_bits"] if bit != "Legal hunting access is not verified here") if validity_read["reason_bits"] else "")),
                "preferred_wind": _wind_from_aspect(sample["aspect_deg"]),
                "best_time_window": "Scout first and verify access",
                "best_time_label": "Terrain-only review",
                "travel_score": int(round(42.0 + sample["local_relief"] * 0.06)),
                "bedding_score": int(round(42.0 + elev_norm * 8.0)),
                "feeding_score": int(round(40.0 + (1.0 - elev_norm) * 8.0)),
                "confidence": int(round(max(32.0, 44.0 + terrain_score * 0.12 - validity_read["confidence_penalty"]))),
                "wind_fit": None,
                "primary_tag": terrain_read["primary_tag"],
                "support_tags": terrain_read["support_tags"],
                "all_tags": terrain_read["all_tags"],
    ...
    alternates = [make_site(i, "alternate", item, f"Alternate Sit #{i}") for i, item in enumerate(selected[1:5], start=2)]

    suppressed_candidates.sort(key=lambda item: item["score"], reverse=True)
    near_misses = [make_site(i, "near-miss", item, f"Near-Miss #{i}") for i, item in enumerate(suppressed_candidates[:3], start=len(alternates) + 2)]
