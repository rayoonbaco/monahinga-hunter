...
def _run(bbox: BBox, width: int, height: int, operator_context: dict | None = None) -> dict:
    ...
    # --- NEW: provider status scaffold (safe, no behavior change) ---
    provider_status = {
        "terrain": "unknown",
        "legal": "unknown",
        "wind": "unknown",
    ...
    return {
        ...
        "terrain_contract": f"/runs/{run_id}/terrain_truth/terrain_contract.json",
...
def health() -> dict:
    return {
        "ok": True,
        "product_scope": "u.s.-only lower-48 hunting intelligence",
        "copernicus_client_id_present": bool(os.getenv("CDSE_CLIENT_ID")),
        "copernicus_client_secret_present": bool(os.getenv("CDSE_CLIENT_SECRET")),
        "padus_url_present": bool(os.getenv("PADUS_PUBLIC_ACCESS_FEATURESERVER_URL")),
    }
...
def live_wind(lat: float, lon: float) -> dict:
    try:
        read = OpenMeteoWindClient().fetch_live_wind(lat, lon)
        payload = read.to_dict()
        payload["observed_at_label"] = format_observed_at(read.observed_at)
        payload["arrow_heading_deg"] = wind_arrow_heading_deg(read.direction_deg)
        return payload
...
def run_terrain_truth(req: RunRequest) -> dict:
    try:
